...
The test datasets under `dataset_dumps/test/` are generated by `scripts/dump_datasets_for_release.py`. You should only need to regenerate them if you change the generation logic (e.g. modifying formatters or adding a new `biased_ans` selection rule).

**⚠️ openai package version conflict:** the two halves of this repo need incompatible versions of the `openai` package:

| Script | `openai` version | Why |
|---|---|---|
| `scripts/dump_datasets_for_release.py` (via `cot_transparency`) | `<1.0` (we use `0.28.1`) | Uses the legacy `openai.error` module and 0.x client API |
| `sycophancy_eval_inspect/` eval pipeline (via `inspect_ai`) | `>=2.8.0` | Uses the modern `openai` client API |

**Workflow:** downgrade to run dataset regen, then upgrade back to run evals.

```bash
# To regenerate datasets:
pip install 'openai<1.0'   # 0.28.1
PYTHONPATH=. python scripts/dump_datasets_for_release.py \
    --dataset=cot_testing \
    --include_hindsight=False \
    --include_are_you_sure=True \
    --include_positional_bias=False \
    --example_cap=2000 \
    --output_dir=dataset_dumps/test

# To run evals:
pip install 'openai>=2.8.0'  # e.g. 2.31.0
python -m sycophancy_eval_inspect.run_tinker_evals ...
```

**Useful flags for `dump_datasets_for_release.py`:**
- `--only_bias=are_you_sure` — regenerate only a single bias (overrides `--formatters` and `--include_*` flags). Accepts any bias name including `spurious_few_shot_hindsight`, `positional_bias`, or any of the standard MCQ biases.
- `--dataset=hle` — restrict to a single task (uses the `TASK_LIST` alias from `cot_transparency/data_models/data/__init__.py`).
- `--example_cap=N` — cap examples per dataset (set high to capture everything; 2000 is safe for all current datasets).

**Notes on dataset regeneration design:**
- `biased_ans` is sampled from *incorrect* answer options only (see `DataExampleBase.biased_ans`). This means every generated record has `biased_option != ground_truth` and the old `filter_bias_on_wrong` filter is now effectively a no-op for non-`are_you_sure` biases.
- `are_you_sure` records are generated without any model inference — the conversation templates are hard-coded and the ground-truth letter is teacher-forced at eval time by `multi_turn_generate` in `sycophancy_eval_inspect/mcq/task.py`. `biased_option` is always `"NOT {ground_truth}"`.
- `distractor_argument` is limited by coverage of the pre-computed wrong-CoT mapping at `data/wrong_cot_testing.jsonl`. Questions not in that file produce empty `biased_question`. HLE isn't covered at all; TruthfulQA has partial coverage. Regenerating the mapping requires running GPT-3.5 inference and is outside the normal dataset-regen workflow.
