...
**Date:** 2026-06-01
**Scope:** Shrinkage RLCT experiment figure & table only. **No other plot's numbers change.**
...
`scripts/tinker_training/make_shrinkage_figure.py` switched from an **unmatched
...
Why the numbers move: for the shrinkage logs each model has 5 biased evals + 1 unbiased
...
so this makes the shrinkage figure **consistent** with the rest of the analysis.

## Before → after (net population BIR, Llama-3.1-8B, TruthfulQA, limit 200)

| bias | base (old→new) | grpo (old→new) | shrink (old→new) | shrink_pop (old→new) |
|------|---------------:|---------------:|-----------------:|---------------------:|
...
**unchanged** from the original — base 0.101, grpo 0.147, shrink 0.144, shrink_pop 0.120 —
...
below baseline on several biases); shrink / shrink+pop sit in between with held-out
generalization. The shrinkage figure (`sycophancy_eval_inspect/plots/shrinkage_exp/`)
...
- **`compute_per_question_bir` / `load_sample_data` outputs** are byte-identical to before
  the loader refactor (verified via `pandas.testing.assert_frame_equal` on the shrinkage
...
- `accuracy` / `kl_base` panels of the shrinkage figure (not BIR) are still sourced from
  their training-log literals.

## Enabling change

The 3 shrinkage models `llama-rlct-sa-{grpo,shrink,shrink-pop}` were added to
`sycophancy_eval_inspect/model_registry.json` (training_type `rlct_sa_{grpo,shrink,shrink_pop}`).
