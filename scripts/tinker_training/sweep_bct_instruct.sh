#!/bin/bash
#
# BCT instruct vs no-instruct sweep (2 configs).
# PREREQUISITE: Run setup_bct_sweep_data.sh first.
#
# Usage: sbatch scripts/tinker_training/sweep_bct_instruct.sh

#SBATCH --account=eecs542w26s001_class
#SBATCH --job-name=sweep-bct-inst
...
#SBATCH --output=logs/sweep-bct-instruct/sweep-slurm-%j.log
...
mkdir -p logs/sweep-bct-instruct

CONFIGS=(sweep_bct_no_instruct sweep_bct_with_instruct)

echo "=== BCT Instruct Sweep: ${#CONFIGS[@]} configs ==="
echo "Started: $(date)"
echo ""
