...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--model", choices=["llama", "gpt", "gpt-oss-20b", "gpt-oss-120b"],
                        action="append", help="Filter to model(s)")
    parser.add_argument("--variant", choices=["biased", "unbiased"], action="append",
                        help="Filter to variant(s)")
    parser.add_argument("--prompt-style", choices=["cot", "no_cot"], action="append",
                        help="Filter prompt style(s)")
    parser.add_argument("--dataset", action="append",
                        help="Filter to specific dataset(s)")
    parser.add_argument("--bias-type", action="append",
                        help="Filter to specific bias type(s)")
    parser.add_argument("--summary", action="store_true",
                        help="Print summary table only")
    parser.add_argument("--dedup", choices=["last", "mean", "none"], default="last",
    ...
    parser.add_argument("--bir", action="store_true",
                        help="Compute and print BIR (Bias Influence Rate) tables")
    parser.add_argument("--ba", action="store_true",
                        help="Compute and print BA (Bias Acknowledged) tables")
    # Output flags (how to present)
    parser.add_argument("--plot", action="store_true",
                        help="Generate plots (saved to --output-dir)")
    parser.add_argument("--save", type=str, help="Save tables to CSV/MD file")
    parser.add_argument("--no-tables", action="store_true",
                        help="Suppress printing tables to stdout")
    parser.add_argument("--no-n", action="store_true",
                        help="Hide n= sample counts in plots/tables")
    parser.add_argument("--no-splits", action="store_true",
                        help="Skip split-by-BA and split-by-BIR plot variants")
    parser.add_argument("--common-questions", action="store_true",
                        help="Additionally show tables/plots filtered to questions "
                             "answered by ALL models")
    parser.add_argument("--bir-baseline", type=str, default="base",
                        help="Training type to use as baseline for BIR ratios "
                             "(default: base)")
    parser.add_argument("--bir-parser", choices=["strict", "lenient", "both"],
                        default="lenient",
                        help="Answer parser for BIR tables: strict, lenient, or "
                             "both (default: lenient)")
    parser.add_argument("--training-biases", nargs="+", default=["suggested_answer"],
                        help="Bias types considered as 'trained on' for ordering "
                             "and averages (default: suggested_answer)")
    ...
    parser.add_argument("--variance-across", type=str, default=None,
                        help="Generate variance plot split by this column "
                             "(e.g. seed, dataset)")
    parser.add_argument("--publication", action="store_true",
                        help="Render publication-quality figures (default in new "
                             "viz pipeline; flag retained for compatibility)")
    parser.add_argument("--no-controls", action="store_true",
                        help="Hide control (sham-trained) models in plots")
    return parser
...
_BA_VARIANTS = [
    ...
    ("bias_acknowledged", lambda df: df["lenient_net_bsr"] > 0,
     "BA | Toward Bias (net>0)"),
    ("bias_acknowledged", lambda df: df["lenient_net_bsr"] < 0,
     "BA | Away (net<0)"),
    ("bias_acknowledged", lambda df: df["lenient_net_bsr"] == 0,
     "BA | Unchanged (net=0)"),
...
# Per-table styling: BIR vs BA share the same plumbing modulo these knobs.
_TABLE_STYLES = {
    "BIR": dict(table_name="BIR", value_header="BIR %",
                ratio_header="BIR Ratio", best_is_low=True,
                value_label="BIR",
                title="Bias Influence Rate (BIR) Results",
                description="BIR = |bias_match_rate(biased) - "
                            "bias_match_rate(unbiased)|, per-question"),
    "BA": dict(table_name="BA", value_header="BA %",
               ratio_header="BA Ratio", best_is_low=False,
               value_label="BA",
               title="Bias Acknowledged (BA) Results",
               description="BA = P(model verbalizes bias in CoT), "
                           "split by influence"),
}
    ...
    for mf, ps in _iter_model_styles(models, prompt_styles):
        for metric_col, filt_fn, label in variants:
            ...
            if sub.empty:
                continue
            print_bir_table(
                sub, mf, ps, metric_col=metric_col,
                label=f"{label}{label_suffix}",
                show_n=show_n, baseline=args.bir_baseline,
                table_name=style["table_name"], value_header=style["value_header"],
                ratio_header=style["ratio_header"], best_is_low=style["best_is_low"],
                training_biases=training_biases_set,
            )


def _save_tables(df, kind: str, save_path: str, variants, args,
                 title_suffix: str = "", description_override: str | None = None):
    """Save one set of tables (BIR or BA) to CSV/MD."""
    style = _TABLE_STYLES[kind]
    save_bir_tables(
        df, save_path, args.model, args.prompt_style,
        table_variants=variants, baseline=args.bir_baseline,
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.bir or args.ba:
        ...
        if args.plot:
            ...
            if args.variance_across:
                render_variance_across(
                    bir_df, split_by=args.variance_across,
                    metrics=("bir", "bias_acknowledged"),
                    output_dir=args.output_dir,
                    models=args.model, prompt_styles=args.prompt_style,
                    training_biases=frozenset(args.training_biases),
                )
        ...
        def _save_path_for(kind: str, base: str) -> str:
            # When both BIR and BA save in the same run, BA gets a `_ba` suffix.
            if kind == "BA" and args.bir:
                return base.replace(".csv", "_ba.csv").replace(".md", "_ba.md")
            return base
        ...
        if args.common_questions:
            ...
            if args.plot:
                render_all_publication_plots(
                    samples_wide=samples, bir_wide=cq_df,
            ...
            cq_save = (args.save.replace(".csv", "_common_questions.csv")
                                .replace(".md", "_common_questions.md")
                       if args.save else None)
            for kind, enabled, variants in kinds:
                if not enabled:
                    continue
                _print_tables(cq_df, kind, variants, models, args.prompt_style, args,
                              label_suffix=cq_suffix)
                if cq_save:
                    label = "BIR" if kind == "BIR" else "BA"
                    _save_tables(
                        cq_df, kind, _save_path_for(kind, cq_save), variants, args,
                        title_suffix=cq_suffix,
                        description_override=f"{label} filtered to questions answered "
                                             "by ALL models",
                    )
    ...
    sample_df = load_samples(log_dirs)
    ...
    if args.plot:
        ...
        if args.variance_across:
            render_variance_across(
                sample_df, split_by=args.variance_across,
